(() => {
  ...
  let selected = localStorage.getItem(STORAGE_KEY) || 'easy';
  ...
  MZV.HordeSystem.prototype.startLevel = function(...args) {
    ...
    const s = this.state;
    if (s.mission === 'thriller' && s.level === 13) {
      const d = cfg();
  ...
  MZV.HordeSystem.prototype.onMusicCue = function(cue) {
    const s = this.state;
    if (s.mission === 'thriller' && s.level === 13 && s.pendingBossVariant === 'shadowDancer' && s.shadowPrepUntil > s.elapsed) {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
      const s = app.state;
      if (!s.running || s.mission !== 'thriller' || s.level !== 13) return;
